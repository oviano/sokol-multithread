...
SOKOL_API_IMPL void simgui_setup(const simgui_desc_t* desc) {
    ...
    */
    ...
    _simgui.vbuf = _simgui.desc.renderer->add_command_make_buffer(&vb_desc);
    ...
    _simgui.ibuf = _simgui.desc.renderer->add_command_make_buffer(&ib_desc);
    ...
    if (!_simgui.desc.no_default_font) {
        ...
        _simgui.img = _simgui.desc->renderer->add_command_make_image(&img_desc);
    ...
    #endif
    _simgui.shd = _simgui.desc.renderer->add_command_make_shader(shd_desc);
    ...
    _simgui.pip = _simgui.desc.renderer->add_command_make_pipeline(pip_desc);

    _simgui.desc.renderer->add_command_pop_debug_group();
}
...
SOKOL_API_IMPL void simgui_shutdown(void) {
    ...
    _simgui.desc.renderer->add_command_push_debug_group("sokol-imgui");
    _simgui.desc.renderer->add_command_destroy_pipeline(_simgui.pip);
    _simgui.desc.renderer->add_command_destroy_shader(_simgui.shd);
    _simgui.desc.renderer->add_command_destroy_image(_simgui.img);
    _simgui.desc.renderer->add_command_destroy_buffer(_simgui.ibuf);
    _simgui.desc.renderer->add_command_destroy_buffer(_simgui.vbuf);
    _simgui.desc.renderer->add_command_pop_debug_group();
}
...
SOKOL_API_IMPL void simgui_render(void) {
    ...
    _simgui.desc.renderer->add_command_apply_viewport(0, 0, fb_width, fb_height, true);
    _simgui.desc.renderer->add_command_apply_scissor_rect(0, 0, fb_width, fb_height, true);

    _simgui.desc.renderer->add_command_apply_pipeline(_simgui.pip);
    _simgui_vs_params_t vs_params;
    vs_params.disp_size.x = io->DisplaySize.x;
    vs_params.disp_size.y = io->DisplaySize.y;
    _simgui.desc.renderer->add_command_apply_uniforms(SG_SHADERSTAGE_VS, 0, &vs_params, sizeof(vs_params));
    sg_bindings bind;
        ...
        */
        ...
        for (int cmd_index = 0; cmd_index < num_cmds; cmd_index++) {
            ...
            if (pcmd->UserCallback) {
				...
				_simgui.desc.renderer->add_command_apply_scissor_rect(scissor_x, scissor_y, scissor_w, scissor_h, true);
                _simgui.desc.renderer->add_command_draw(base_element, pcmd->ElemCount, 1);
            }
